...
**MSPDF** is a powerful, privacy-first, client-side PDF toolkit that is self hostable and allows you to manipulate, edit, merge, and process PDF files directly in your browser. No server-side processing is required, ensuring your files remain secure and private.

[![Docker Downloads](https://img.shields.io/badge/dynamic/json?url=https%3A%2F%2Fghcr-badge.elias.eu.org%2Fapi%2Falam00000%2FMSpdf%2FMSpdf&query=%24.downloadCount&logo=docker&label=Docker%20Downloads&color=blue)](https://github.com/alam00000/MSpdf/pkgs/container/MSpdf) [![Ko-fi](https://img.shields.io/badge/Buy%20me%20a%20Coffee-yellow?logo=kofi&style=flat-square)](https://ko-fi.com/alio01) ![GitHub Stars](https://img.shields.io/github/stars/alam00000/MSpdf?style=social)
[![Sponsor me on GitHub](https://img.shields.io/badge/Sponsor-%E2%9D%A4-ff69b4)](https://github.com/sponsors/alam00000)

![MSPDF Tools](public/images/MSpdf-tools.png)
...
[![Documentation](https://img.shields.io/badge/Docs-VitePress-646cff?style=for-the-badge&logo=vite&logoColor=white)](https://MSpdf.com/docs/)

Visit our [Documentation](https://MSpdf.com/docs/) for:
...
MSPDF is **dual-licensed** to fit your needs:
...
📖 For more details, see our [Licensing Page](https://MSpdf.com/licensing.html)

### AGPL Components (Pre-configured via CDN)

MSPDF does **not** bundle AGPL-licensed processing libraries in its source code, but **pre-configures CDN URLs** so all features work out of the box with zero setup:
...
[![Star History Chart](https://api.star-history.com/svg?repos=alam00000/MSpdf&type=Date)](https://star-history.com/#alam00000/MSpdf&Date)

---

## 💖 Thank You to Our Sponsors

We're incredibly grateful to all our sponsors and supporters who help keep MSPDF free and open source!
...
- **High Performance**: Built with modern web technologies, MSPDF is fast and efficient, handling even large PDF files with ease.
- **Completely Free**: MSPDF is a free and open-source tool for everyone.

---

## 🛠️ Features / Tools Supported

MSPDF offers a comprehensive suite of tools to handle all your PDF needs.
...
MSPDF is available in multiple languages:
...
Want to help translate MSPDF into your language? Check out our [Translation Guide](TRANSLATION.md)!

---

## 🚀 Getting Started

You can run MSPDF locally for development or personal use.
...
Run MSPDF instantly from GitHub Container Registry (Recommended):
...
```bash
docker run -p 3000:8080 MSpdfteam/MSpdf:latest
```

**Podman (GHCR):**

```bash
podman run -p 3000:8080 ghcr.io/alam00000/MSpdf:latest
```

**Podman (Docker Hub):**

```bash
podman run -p 3000:8080 docker.io/MSpdfteam/MSpdf:latest
```
...
It is very straightforward to host your own instance of MSPDF using a static web page hosting service. Plus, services such as Netlify, Vercel, and GitHub Pages all offer a free tier for getting started. See [Static Hosting](https://github.com/alam00000/MSpdf/blob/main/STATIC-HOSTING.md) for details.

### 🏠 Self-Hosting Locally

Since MSPDF is fully client-side, all processing happens in the user's browser and no server-side processing is required. This means you can host MSPDF as simple static files on any web server or hosting platform.

**Download from Releases (Recommended):**

The easiest way to self-host is to download the pre-built distribution file from our [GitHub releases](https://github.com/alam00000/MSpdf/releases). Each release includes a `dist-{version}.zip` file that contains all necessary files for self-hosting.

1. Go to [MSPDF Releases](https://github.com/alam00000/MSpdf/releases)
...
MSPDF supports different compression modes for optimized builds:

```bash
# Gzip only (smallest Docker image size)
npm run build:gzip
docker build --build-arg COMPRESSION_MODE=g -t MSpdf:gzip .

# Brotli only (best compression ratio)
npm run build:brotli
docker build --build-arg COMPRESSION_MODE=b -t MSpdf:brotli .

# No compression (fastest build time)
npm run build:original
docker build --build-arg COMPRESSION_MODE=o -t MSpdf:original .

# All formats (default, maximum browser compatibility)
npm run build:all
docker build --build-arg COMPRESSION_MODE=all -t MSpdf:all .
```
...
MSPDF can use jsDelivr CDN to serve large WASM files (LibreOffice, Ghostscript, PyMuPDF) for improved performance and reduced bandwidth costs:
...
```bash
VITE_WASM_PYMUPDF_URL=https://cdn.jsdelivr.net/npm/@MSpdf/pymupdf-wasm@0.11.14/
VITE_WASM_GS_URL=https://cdn.jsdelivr.net/npm/@MSpdf/gs-wasm/assets/
...
- `MSpdf.tar` — the Docker image
- `MSpdf-pymupdf-wasm-0.11.14.tgz` — PyMuPDF WASM package
- `MSpdf-gs-wasm-*.tgz` — Ghostscript WASM package
...
Users open their browser, access MSPDF on the internal network, and the browser fetches WASM files from the internal server. No internet required at any point.
...
MSPDF can also be hosted from a subdirectory (e.g., `example.com/tools/MSpdf/`):

```bash

# Example:
# 1. Build the app with the specific BASE_URL. BASE_URL must have a trailing and leading slash. The BASE_URL can be any url of your choice. Here we are using /tools/MSpdf/ as an example.

BASE_URL=/tools/MSpdf/ npm run build
...
mkdir -p serve-test/tools/MSpdf

# 3. Copy all files from the 'dist' folder into that nested directory
cp -r dist/* serve-test/tools/MSpdf/

# 4. Serve the 'serve-test' folder
npx serve serve-test
```

The website can be accessible at: `http://localhost:3000/tools/MSpdf/`
...
MSPDF's Docker image also supports the `BASE_URL` build argument for subdirectory deployments:

```bash
# Build for subdirectory deployment
docker build --build-arg BASE_URL=/MSpdf/ -t MSpdf .

# Run the container
docker run -p 3000:8080 MSpdf

# The app will be accessible at http://localhost:3000/MSpdf/
```
...
> [!IMPORTANT]
>
> - Always include trailing slashes in `BASE_URL` (e.g., `/MSpdf/` not `/MSpdf`)
...
services:
  MSpdf:
    image: ghcr.io/alam00000/MSpdf:latest # Recommended
    # image: MSpdfteam/MSpdf:latest     # Alternative: Docker Hub
...
For Linux production deployments, you can run MSPDF as a systemd service using [Podman Quadlet](https://docs.podman.io/en/latest/markdown/podman-systemd.unit.5.html).

Create `~/.config/containers/systemd/MSpdf.container`:
...
systemctl --user enable --now MSpdf
```

For detailed Quadlet configuration, see [Self-Hosting Docker Guide](https://MSpdf.com/docs/self-hosting/docker).

### 🏢 Simple Mode for Internal Use

For organizations that want a clean, distraction-free interface focused solely on PDF tools, MSPDF supports a **Simple Mode** that hides all branding and marketing content.
...
MSPDF runs as a non-root user using nginx-unprivileged for enhanced security:
...
```bash
docker build -t MSpdf .
docker run -p 8080:8080 MSpdf
```
...
4. **Note your worker URL** (e.g., `https://MSpdf-cors-proxy.your-subdomain.workers.dev`)
...
MSPDF uses client-side HMAC as a deterrent against casual abuse, but accepts this tradeoff due to its fully client-side architecture. To enable:
...
MSPDF supports semantic versioning with multiple container tags available:

**GitHub Container Registry (Recommended):**

- **Latest**: `ghcr.io/alam00000/MSpdf:latest`
- **Specific Version**: `ghcr.io/alam00000/MSpdf:1.0.0`
- **Version with Prefix**: `ghcr.io/alam00000/MSpdf:v1.0.0`

**Docker Hub:**

- **Latest**: `MSpdfteam/MSpdf:latest`
- **Specific Version**: `MSpdfteam/MSpdf:1.0.0`
- **Version with Prefix**: `MSpdfteam/MSpdf:v1.0.0`
...
MSPDF was originally built using **HTML**, **CSS**, and **vanilla JavaScript**. As the project grew, it was migrated to a modern stack for better maintainability and scalability:
...
Have an idea for a new tool or an improvement? [Open an issue](https://github.com/alam00000/MSpdf/issues) to discuss it first.
...
MSPDF wouldn't be possible without the amazing open-source tools and libraries that power it. We'd like to extend our heartfelt thanks to the creators and maintainers of:
...
> [!NOTE]
> AGPL-licensed libraries are not bundled in MSPDF's source code. They are loaded at runtime from CDN (pre-configured) and can be overridden via environment variables or Advanced Settings.
