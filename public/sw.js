      ...
      .then((cacheNames) => {
        return Promise.all(
          cacheNames.map((cacheName) => {
            if (cacheName.startsWith('MSpdf-') && cacheName !== CACHE_NAME) {
...
function shouldCache(pathname, isCDN = false) {
  if (isCDN) {
    return (
      pathname.includes('/@MSpdf/pymupdf-wasm') ||
      pathname.includes('/@MSpdf/gs-wasm') ||
